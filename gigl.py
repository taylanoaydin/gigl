...
def send_application(to_email, subject, userName, candidateName, gigTitle, applicationMessage, gigID):
    msg = Message(subject,
                  sender=current_app.config['MAIL_USERNAME'],
                  recipients=[to_email])
    msg.html = flask.render_template('applicationEmail.html', userName=userName, candidateName=candidateName, gigTitle=gigTitle, applicationMessage=applicationMessage, gigID=gigID)
...
def details(id):
    ...
    if apply_form.validate_on_submit():
        ...
        if database.owns_gig(netid, id):
            ...
            send_application(gigNetID+ "@princeton.edu", "You have a new application!", database.get_user(gigNetID).get_name(), database.get_user(netid).get_name(), gigTitle, application_message, id)
        else:
